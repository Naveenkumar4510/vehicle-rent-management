...
public class AddFeedback implements HttpRequestHandler 
{
	
	static Logger log = Logger.getLogger(AddFeedback.class);
...
//		user.setTechnology(request.getParameter("tech"));
		...
		fb.setUserid(Integer.parseInt(request.getParameter("userid")));
		fb.setVno(Integer.parseInt(request.getParameter("vno")));
...
//				setPassword("NttData@"+ Integer.toString(user.getStuid());
		...
		*/
        int finalresult;
        try {
            finalresult = dao.insertFeedback(fb);
            if (finalresult > 0) {
                out.println("<html>");
                out.println("<head>");
                out.println("<title>Feedback Inserted Successfully</title>");
                out.println("<style>");
                out.println("body {");
                ...
                out.println("</head>");
                out.println("<body>");
                out.println("<h1>Feedback Inserted Successfully</h1>");
                out.println("<div class=\"success-message\">Feedback Inserted Successfully!</div>");
                out.println("<div class=\"button-container\">");
                out.println("<a class=\"link\" href=\"home.jsp\">HOME PAGE</a>");
